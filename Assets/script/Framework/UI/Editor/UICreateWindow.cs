...
public class UICreateWindow : EditorWindow
{
    ...
    private void SaveJson()
    {
        ...
        File.WriteAllText(UIConfig, JsonUtility.ToJson(new UIConfigJsonList { items = list }, true));
    }
    ...
    private UIConfigJson GetUIJson(string name)
    {
        if (uiJsonDatas.Count == 0)
        {
            try
            {
                ...
                var wrapper = JsonUtility.FromJson<UIConfigJsonList>(json);
                if (wrapper == null || wrapper.items == null)
                {
                    Debug.LogError("UIConfig.json 解析失败：" + UIConfig);
                    return null;
                }
